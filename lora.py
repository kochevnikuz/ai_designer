    ...
    def handle(self, *args, **options):
        ...
        for filename in os.listdir(input_dir):
            ...
            try:
                ...
                self._save_sample(img_resized, output_dir, f"{base_name}_full", trigger_word, "full carpet composition, central medallion, borders")
                count += 1
                ...
                if w >= 512 and h >= 512:
                    ...
                    self._save_sample(img_center, output_dir, f"{base_name}_center", trigger_word, "carpet central pattern detail, intricate ornaments")
                    count += 1
                    
                    # Левый верхний угол (бордюр)
                    img_corner = img.crop((0, 0, 512, 512))
                    self._save_sample(img_corner, output_dir, f"{base_name}_corner", trigger_word, "carpet border detail, corner pattern")
                    count += 1
